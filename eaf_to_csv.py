...
def extract_first_matching_action(eaf, tiers_actions, start, end):
    """
    For a given time window [start, end] in ms:
    - Scan each tier in tiers_actions
    - If any annotation has value 'yes' and overlaps, return that tier's label
    """
    for tier_name, label in tiers_actions:
        if tier_name in eaf.tiers:
            annots = eaf.get_annotation_data_for_tier(tier_name)
            for t0, t1, val in annots:
                if val and str(val).lower() == "yes":
                    # Overlap condition: not completely before or after
                    if not (t1 <= start or t0 >= end):
                        return label
    return "unknown"


def extract_multiclass_label(eaf, tier_name, start, end):
    """
    For tiers that directly store the category label (e.g., 'close + facing adult').
    Return the first overlapping label, or 'unknown' if none.
    """
...
def process_time_segment(eaf, start_sec, end_sec):
    """
    ...
    for tier_name, outcol in MULTICLASS_TIERS.items():
        raw_val = extract_multiclass_label(eaf, tier_name, start_ms, end_ms)
...
def main():
    ...
    for _, row in template.iterrows():
        ...
        # Only keep the segment within [START_SEC, END_SEC]
        if t0 < START_SEC or t0 > END_SEC:
            continue

        human = process_time_segment(eaf, t0, t1)
